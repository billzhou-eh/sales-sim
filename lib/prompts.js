...
function buildChoiceGeneratorPrompt(persona, scenario, messages, stage) {
  ...
  const recentConvo = messages.slice(-6).map(m =>
    (m.role === 'customer' ? persona.name : 'Salesperson') + ': ' + m.text
  ).join('\n');

  // Per-stage: what type the excellent and good choices should be, and any extra constraint
  const stageConfig = {
    'open':       { hint: 'Opening — build credibility, earn the right to ask questions, set the agenda. Do NOT ask about problems yet.', excellentType: 'rapport',    goodType: 'situation' },
    'probe':      { hint: 'Problem questions — surface a specific operational friction the customer lives with. Ask what goes wrong, what is difficult, what they wish were different.', excellentType: 'problem',    goodType: 'situation' },
    'develop':    { hint: 'Implication questions — explore the consequences, cost, or knock-on effects of the problem already identified.', excellentType: 'implication', goodType: 'problem'   },
    'quantify':   { hint: 'Implication/quantify — ask for concrete figures on impact: time lost, cost, frequency, risk level.', excellentType: 'implication', goodType: 'problem'   },
    'needpayoff': { hint: 'Need-payoff — ask what it would be worth to solve this, or what a fix would enable. Get the customer to state the value themselves.', excellentType: 'needpayoff',  goodType: 'implication' },
    'close':      { hint: 'Agree a next step — the excellent option MUST be a statement that proposes a specific, low-risk next step the customer can say yes to (e.g. "I\'d like to show you...", "Would it make sense to schedule a demo?", "Can I arrange a brief trial?"). It must NOT be another question about pain or impact — the pain conversation is over. The good option may be a need-payoff question. The poor option pitches product features.', excellentType: 'needpayoff', goodType: 'needpayoff' },
  };
  const cfg = stageConfig[stage.id] || { hint: stage.title, excellentType: 'other', goodType: 'other' };
...
- "excellent" (10 pts): best move — type must be "${cfg.excellentType}"
- "good" (5 pts): acceptable but less sharp — type must be "${cfg.goodType}"
- "poor" (0 pts): wrong technique — premature product pitch, too vague, assumptive, or off-topic — type must be "solution"

Output ONLY valid JSON — no markdown, no fences, no explanation:
{"choices":[
  {"id":"a","text":"...","quality":"excellent","type":"${cfg.excellentType}","rationale":"One sentence why.","points":10},
  {"id":"b","text":"...","quality":"good","type":"${cfg.goodType}","rationale":"One sentence why.","points":5},
